...
import com.artbridge.exhibition.config.EmbeddedKafka;
import com.artbridge.exhibition.config.KafkaSseConsumer;
import com.artbridge.exhibition.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class ExhibitionKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/exhibition-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/exhibition-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/exhibition-kafka/unregister"));
                return;
            }
        }
